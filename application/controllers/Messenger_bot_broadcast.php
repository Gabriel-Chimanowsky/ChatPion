...
class Messenger_bot_broadcast extends Home
{
    ...
    public function otn_subscriber_broadcast_campaign()
    {
        ...
        $page_info = []; // Inicializa como array vazio para evitar erro de undefined variable
        foreach ($page_list as $value) {
            if (!empty($this->team_allowed_pages) && !in_array($value['id'], $this->team_allowed_pages))
                continue;
    ...
    public function otn_subscriber_broadcast_campaign_data()
    {
        ...
        if ($status != "")
            $this->db->where(array("posting_status" => $status));
        ...
        foreach ($info as $key => $value) {
            ...
            if ($info[$key]['schedule_time'] != "0000-00-00 00:00:00")
                ...
                $scheduled_at = '<span class="text-muted"><i class="fas fa-exclamation-circle"></i> ' . $this->lang->line("Not Scheduled") . "<span>";
            ...
            if ($posting_status == '1')
                ...
                $info[$key]['delete'] = "<a class='btn btn-circle btn-outline-danger delete'  id='" . $info[$key]['id'] . "' data-toggle='tooltip' title='" . $this->lang->line("Delete Campaign") . "' href=''><i class='fas fa-trash-alt'></i></a>";
            ...
            if ($this->config->item("broadcaster_number_of_message_to_be_sent_in_try") == "" || $this->config->item("broadcaster_number_of_message_to_be_sent_in_try") == "0") {
                ...
                $action_count++;
                if ($posting_status == '3')
                    $force_porcess_str .= "<a href='' class='btn btn-circle btn-outline-success play_campaign_info' data-toggle='tooltip' title='" . $this->lang->line("Resume Campaign") . "' table_id='" . $info[$key]['id'] . "'><i class='fas fa-play'></i></a>";
                else if ($posting_status != '4')
                    $force_porcess_str .= "<a href='' class='btn btn-circle btn-outline-dark pause_campaign_info' data-toggle='tooltip' title='" . $this->lang->line("Pause Campaign") . "' table_id='" . $info[$key]['id'] . "'><i class='fas fa-pause'></i></a>";
            }
            ...
            if ($posting_status == '2')
                $info[$key]['posting_status'] = '<div style="min-width:100px"><span class="text-success badge"><i class="fas fa-check-circle"></i> ' . $this->lang->line("Completed") . '</span></div>';
            else if ($posting_status == '1')
                $info[$key]['posting_status'] = '<div style="min-width:100px"><span class="text-warning"><i class="fas fa-spinner"></i> ' . $this->lang->line("Processing") . '</span></div>';
            else if ($posting_status == '3')
                $info[$key]['posting_status'] = '<div style="min-width:100px"><span class="text-muted"><i class="fas fa-stop"></i> ' . $this->lang->line("Paused") . '</span></div>';
            else if ($posting_status == '4')
                $info[$key]['posting_status'] = '<div style="min-width:100px"><span class="text-dark"><i class="fas fa-ban"></i> ' . $this->lang->line("On-hold") . $hold_message . '</span></div>';
            else
                $info[$key]['posting_status'] = '<div style="min-width:100px"><span class="text-danger"><i class="far fa-times-circle"></i> ' . $this->lang->line("Pending") . '</span></div>';
    ...
    public function otn_campaign_sent_status()
    {
        ...
        if ($posting_status == '2')
            $posting_status = '<span class="text-success"> (' . $this->lang->line("Completed") . ')</span>';
        else if ($posting_status == '1')
            $posting_status = '<span class="text-warning"> (' . $this->lang->line("Processing") . ')</span>';
        else if ($posting_status == '3')
            $posting_status = '<span class="text-muted"> (' . $this->lang->line("Paused") . ')</span>';
        else if ($posting_status == '4')
            $posting_status = '<span class="text-dark"> (' . $this->lang->line("On-hold") . ')</span>';
        else
            $posting_status = '<span class="text-danger"> (' . $this->lang->line("Pending") . ')</span>';
    ...
    public function otn_campaign_sent_status_data()
    {
        ...
        foreach ($info as $key => $value) {
            ...
            if ($value["sent_time"] != "0000-00-00 00:00:00")
                $sent = "<i class='fa fa-check-circle green'></i> " . date("M j, y H:i", strtotime($value['sent_time']));
            if ($value["opened"] == "1")
                $opened = "<i class='fa fa-check-circle green'></i> " . date("M j, y H:i", strtotime($value['open_time']));
            if ($value["delivered"] == "1")
                $delivered = "<i class='fa fa-check-circle green'></i> " . date("M j, y H:i", strtotime($value['delivery_time']));
            if ($value["clicked"] == "1")
                $clicked = "<i class='fa fa-check-circle green'></i> " . date("M j, y H:i", strtotime($value['click_time']));
            ...
            if (isset($tempu[0]) && (strlen($tempu[0]) > 50) || strpos($tempu[0], 'mid.$') !== false)
                $msg_sent_id = ' <i class="fa fa-check green"></i> ' . $this->lang->line("sent") . " : " . $value['message_sent_id'];
            else
                $msg_sent_id = $value['message_sent_id'];

            if ($value['message_sent_id'] == "")
                $info[$key]["message_sent_id"] = "<i class='fa fa-remove red'></i>";
            else
                $info[$key]["message_sent_id"] = $msg_sent_id;
    ...
    public function otn_create_subscriber_broadcast_campaign()
    {
        ...
        if ($data['templates'][9] == 'list')
            unset($data['templates'][9]);
    ...
    public function otn_subscriber_bulk_broadcast_add_action()
    {
        ...
        for ($k = 1; $k <= 1; $k++) {
            ...
            if ($template_type == 'carousel') {
                ...
                for ($j = 1; $j <= 10; $j++) {
                    ...
                    if (!isset($$carousel_title) || $$carousel_title == '')
                        continue;
            ...
            if ($template_type == 'list') {
                ...
                for ($j = 1; $j <= 4; $j++) {
                    ...
                    if (!isset($$list_image) || $$list_image == '')
                        continue;
        ...
        if (!empty($label_ids))
            ...
            $insert_data['label_ids'] = "";
        if (!empty($excluded_label_ids))
            $insert_data['excluded_label_ids'] = implode(',', $excluded_label_ids);
        else
            $insert_data['excluded_label_ids'] = "";
        if (!empty($otn_postback_ids))
            $insert_data['otn_postback_id'] = implode(',', $otn_postback_ids);
        else
            $insert_data['otn_postback_id'] = "";
        ...
        if ($broadcast_type == '24H Promo')
            $promo_sql = "last_subscriber_interaction_time > '{$previous_time}' AND";
        if ($broadcast_type == '24+1 Promo')
            $promo_sql = "(last_subscriber_interaction_time < '{$previous_time}' AND is_24h_1_sent='0') AND";
        $this->_time_zone_set();
        //========================================
        ...
        if (count($label_ids) > 0)
            $sql_part = ' messenger_bot_subscribers_label.contact_group_id IN (' . implode(',', $label_ids) . ') AND ';
        if (count($label_ids) > 0)
            $sql_part2 = ' messenger_bot_subscribers_label.contact_group_id NOT IN (' . implode(',', $excluded_label_ids_temp) . ') AND ';

        $sql_part3 = "";
        $sql_part_array3 = array();
        if ($user_gender != '')
            $sql_part_array3[] = "gender = '{$user_gender}'";
        if ($user_time_zone != '')
            $sql_part_array3[] = "timezone = '{$user_time_zone}'";
        if ($user_locale != '')
            $sql_part_array3[] = "locale = '{$user_locale}'";
