...
class Home extends CI_Controller
{
    ...
    {
        parent::__construct();
        // Verifica se a função está disponível (pode estar desabilitada em alguns servidores)
    ...
    protected function system_message_insert_into_conversation($subscriber_id, $page_table_id, $agent_name, $message_content, $social_media = null, $user_id = null)
    {
        ...
        if (!empty($social_media))
            $insert_data['platform'] = $social_media;
        if (!empty($user_id))
    ...
    public function installation_action()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                // Executa migrations customizadas (Chatpeao additions)
                $custom_migrations = [
                    APPPATH . 'migrations/broadcast_pro_tables.sql',
                    APPPATH . 'migrations/broadcast_pro_menu.sql',
                    APPPATH . 'migrations/add_lead_scanner_menu.sql'
                ];
                foreach ($custom_migrations as $migration_path) {
                    if (file_exists($migration_path)) {
                        $this->basic->import_dump($migration_path);
                    }
                }
                // Fim das migrations customizadas
    ...
    public function _subscription_viewcontroller($data = array())
    {
        ...
        if (file_exists(APPPATH . $theme_file_path))
            ...
            $theme_load = "site/modern/subscription_theme";
    ...
    public function login($is_team_login = '0') //loads home view page after login (this )
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($count == 0) {
                ...
                if (isset($package_info_session["id"]))
                ...
                if ($is_team_login == '1') {
                    ...
                    if (isset($role_info_session["id"]))
                        $this->session->set_userdata('current_role_id', $role_info_session["id"]);
    ...
    public function get_broadcast_tags($media_type = 'fb')
    {
        ...
        if (strtotime(date("Y-m-d")) > strtotime("2020-3-4"))
            return $new_tags;
    ...
    public function _theme_list()
    {
        ...
        foreach ($file_list as $file) {
            ...
            if ($pos !== FALSE)
                continue; // only loading unminified css
            if ($theme == "_all-skins.css")
                continue;  // skipping large css file that includes all file
    ...
    public function get_broadcast_summary()
    {
        ...
        if ($is_bot_subscriber == '1')
            ...
            $where_simple2 = array("page_table_id" => $page_id, 'client_thread_id !=' => '', 'user_id' => $this->user_id, 'permission' => '1');
    ...
    public function get_otn_broadcast_summary()
    {
        ...
        if (!isset($otn_postback_ids) || !is_array($otn_postback_ids))
        ...
        if ($load_otn_postback == '1') {
            ...
            foreach ($otn_postback_info as $value) {
                $selected = '';
                if (in_array($value['id'], $postback_ids_array))
                    $selected = 'selected';
        ...
        if ($load_label == '1') {
            ...
            foreach ($info_type as $value) {
                $selected = '';
                if (in_array($value['id'], $label_ids_array))
                    $selected = 'selected';

                $selected2 = '';
                if (in_array($value['id'], $exclude_label_array))
                    $selected2 = 'selected';
        ...
        if ($is_bot_subscriber == '1')
            $where_simple2 = array("messenger_bot_subscriber.page_table_id" => $page_id, 'is_bot_subscriber' => '1', 'unavailable' => '0', 'user_id' => $this->user_id, 'permission' => '1');
        else
            $where_simple2 = array("messenger_bot_subscriber.page_table_id" => $page_id, 'client_thread_id !=' => '', 'user_id' => $this->user_id, 'permission' => '1');
        ...
        if ($load_label == '0') {
            if (count($label_ids) > 0)
                $this->db->where_in('messenger_bot_subscribers_label.contact_group_id', $label_ids);
            if (count($excluded_label_ids) > 0)
                $this->db->where_not_in('messenger_bot_subscribers_label.contact_group_id', $excluded_label_ids);
        }
    ...
    public function get_rcn_broadcast_summary()
    {
        ...
        if ($is_bot_subscriber == '1')
            $where_simple2 = array("messenger_bot_subscriber.page_table_id" => $page_id, 'is_bot_subscriber' => '1', 'unavailable' => '0', 'messenger_bot_subscriber.user_id' => $this->user_id, 'permission' => '1', 'notification_type !=' => 'otn');
        else
            $where_simple2 = array("messenger_bot_subscriber.page_table_id" => $page_id, 'client_thread_id !=' => '', 'messenger_bot_subscriber.user_id' => $this->user_id, 'permission' => '1', 'notification_type !=' => 'otn');
    ...
    protected function get_country_iso_phone_currecncy($return = 'country') // country,currency_name,currecny_icon,phonecode
    {
        ...
        foreach ($countries as $key => $value) {
            if ($return == 'country')
                $output[$value['iso_alpha2']] = $value['name'];
            else if ($return == 'currency_name')
                $output[$value['currency_code']] = $value['currency_code'] . " (" . $value['currency_name'] . ")";
            else if ($return == 'currecny_icon')
                $output[$value['currency_code']] = !empty($value['currency_symbol']) ? $value['currency_symbol'] : $value['currency_code'];
            else
                $output[$value['iso_alpha2']] = $value['calling_code'];
        }
        if (isset($output['']))
            unset($output['']);
    ...
    protected function delete_addon($addon_controller_name = "", $sql = array())
    {
        ...
        try {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                // DESABILITADO - Chamada externa removida para uso local
                // if ($purchase_code != "") {
    ...
    {
        if (!$this->input->is_ajax_request() && $this->strict_ajax_call)
            exit();
    }
    ...
    protected function csrf_token_check($return_bool = false)
    {
        ...
        if (is_null($csrf_token_form) || is_null($csrf_token_session))
            $is_error = true;
        else if (!hash_equals($csrf_token_form, $csrf_token_session))
            $is_error = true;
        ...
        if ($is_error) {
            ...
            if ($this->input->is_ajax_request())
                ...
                redirect('home/error_csrf', 'location');
            exit();
        }
        return true;
    }
    ...
    {
        if ($this->session->userdata('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 66)))
            return true;
        if ($this->session->userdata('user_type') == 'Member' && in_array(335, $this->module_access))
            return true;
        return false;
    }
    ...
    {
        if ($this->session->userdata('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 67)))
            return true;
        if ($this->session->userdata('user_type') == 'Member' && in_array(340, $this->module_access))
            return true;
        return false;
    }
    ...
    {
        if ($this->session->userdata('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 40)))
            return true;
        if ($this->session->userdata('user_type') == 'Member' && (in_array(270, $this->module_access) || in_array(271, $this->module_access)))
            return true;
        return false;
    }
    ...
    protected function ecommerce_exist()
    {
        if ($this->session->userdata('user_type') == 'Admin')
            return true;
        if ($this->session->userdata('user_type') == 'Member' && in_array(268, $this->module_access))
            return true;
        return false;
    }
    ...
    {
        if ($this->session->userdata('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 39)))
            return true;

        if ($this->session->userdata('user_type') == 'Member' && in_array(269, $this->module_access))
            return true;
    ...
    {
        if ($this->basic->is_exist("add_ons", array("unique_name" => $unique_name)))
            return true;
        return false;
    }
    ...
    public function assign_drip_messaging_id($drip_type, $check_box_plugin_id, $PAGE_AUTO_ID, $subscriber_id, $drip_campaign_id = "")
    {
        if (!isset($drip_type))
            $drip_type = "default";
        if (!isset($check_box_plugin_id))
            $check_box_plugin_id = "0";
            ...
            if (isset($login_config[0])) {
                ...
                foreach ($settings_array['google_sheet'] as $key => $value) {
                    ...
                    $google_sheet_account_data = $this->db
                        ...
                        ->get()
                        ->row_array();
                    if (empty($google_sheet_account_data))
                        return false;
    ...
    public function check_all_condition($rules, $condition_type, $subscriber_info, $custom_field_info)
    {
        ...
        foreach ($rules as $key => $value) {
            ...
            if ($condition_type == "or") {
                ...
                if ($final_result)
                    return $final_result; // if 1 at anytime, return
            } else {
                $final_result = isset($final_result) ? ($final_result && $result) : $result;
                if (!$final_result)
                    return $final_result; // If 0 at any time, return
            }
        }
        return $final_result;
    }
    ...
    protected function updated_subscriber_last_interaction($subscriber_id = null, $update_interaction = false, $message_content = null, $last_conversation_message = null, $update_unseen = true)
    {
        if (!empty($subscriber_id) && $subscriber_id != 0) {
            ...
            if ($update_interaction)
                $update_data .= ',last_subscriber_interaction_time="' . $curtime . '"';

            if (!empty($message_content) || !empty($last_conversation_message)) {
                if (empty($last_conversation_message))
                ...
                if ($update_unseen)
                    $update_data .= ',unseen_count=unseen_count+1';
            }
    ...
    {
        if (empty($insert_data) || !isset($insert_data['subscriber_id']) || !isset($insert_data['fb_page_id']))
            return false;
        ...
        if (empty($user_id) || empty($page_id)) {
            ...
            if (!isset($page_info[0]))
                return false;
            if (empty($user_id))
                $user_id = $page_info[0]['user_id'] ?? 0;
            if (empty($page_id))
                $page_id = $page_info[0]['id'] ?? 0;
            $insert_data["user_id"] = $user_id;
        ...
        if ($insert_data['sender'] == 'bot' && $this->session->userdata('logged_in') == 1)
            $insert_data['agent_name'] = $this->session->userdata('username');
        ...
        if ($has_livechat_access) {
            ...
            // dd($currtime);
            if (!isset($insert_data['conversation_time']))
                $insert_data['conversation_time'] = $currtime;
            ...
            if ($sender == 'bot')
                $message_content_html = display_sent_message($page_id, $fb_page_id, $message_content, $message_id, $conversation_time, $agent_name, false, $insert_data['message_status'] ?? null, $insert_data['delivery_status_updated_at'] ?? null);

            if ($sender == 'user')
                $message_content_html = display_received_message($page_id, $fb_page_id, $message_content, $message_id, $conversation_time, false, $insert_data['message_status'] ?? null, $insert_data['delivery_status_updated_at'] ?? null);
    ...
    { // accepts both id and subscriber_id, settings_http_api_data used in sandbox
        ...
        if (empty($subscriber_unique_field))
            $subscriber_unique_field = "subscribe_id";
        ...
        foreach ($find as $fk => $fv) {
            if (isset($replace[$fk]))
                $api_endpoint_url = str_replace($fv, urlencode($replace[$fk]), $api_endpoint_url);
        }
        if ($body_data_type == 'json') {
            ...
            foreach ($body_data as $value) {
                ...
                if ($value["type"] == "static") {
                    ...
                    if ($body_data_type == "binary")
                        $parsed_value = $file_url;
                    else
                        $parsed_value = curl_file_create($file_url);
                }
        ...
        try {
            ...
            if (count($update_subscriber_custom_field_data) > 0 && !empty($custom_field_ids)) {
                ...
                foreach ($update_subscriber_custom_field_data as $row) {
                    if (!isset($custom_field_data_found_mapped[$row[$custom_field_map_custom_field_id_field]]))
                        continue;
                    $check_where = $row;
